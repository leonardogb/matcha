  ...
  if (
    ...
    if (/^[0-9]+$/g.test(ageMin) && /^[0-9]+$/g.test(ageMax)) {
      if (ageMin >= 18 && ageMin <= 99 && ageMax >= 18 && ageMax <= 99) {
        if (ageMin <= ageMax) return { ageMin: ageMin, ageMax: ageMax };
        else return { ageMin: ageMax, ageMax: ageMin };
      }
    }
  }
  return false;
  ...
  if (
    ...
    if (/^[0-9]+$/g.test(populMin) && /^[0-9]+$/g.test(populMax)) {
      if (
        ...
        else return { populMin: populMax, populMax: populMin };
      }
    }
  }
  return false;
    ...
    userModel
      ...
      .then((user1) => {
        if (user1) {
          ...
          if (
            req.body.tri == "age" ||
            req.body.tri == "location" ||
            req.body.tri == "popularite" ||
            req.body.tri == "tags"
          )
            tri = req.body.tri;
          else tri = "default";
          if (age && popul) {
            if (req.body.recherche == "normal") {
                    ...
                    userTab.forEach((element) => {
                      ...
                      Promise.all(puntos)
                        .then((punts) => {
                          ...
                          if (++i == userTab.length) {
                            if (tri == "default") usuarios.sort(compareMatch);
                            else if (tri == "tags") usuarios.sort(compareTags);
                            res.send(usuarios);
                          }
                        })
                        .catch(function (err) {
              ...
              userModel
                .getUsersLimits2(user1.id, age, popul, tri)
                .then((userTab) => {
                  if (userTab && userTab.length > 0) {
                    //console.log(userTab);
                    var usuarios = [];
