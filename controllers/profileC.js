...
router.post("/update", function (req, res) {
  ...
  if (update != true) {
    ...
    profileModel.update(tab).then((respuesta) => {
      ...
      } else req.session.user.error = "Votre profil n'est pas a jour";
  ...
  if (
    ...
    if (validUsername != true)
      ...
      req.session.user.error =
        "Votre mot de passe doit avoir des chiffres, des lettres, un caractère special [@$!%*#?&] et au moins 8 caractères";
...
router.post("/update/images", function (req, res) {
  ...
  if (!fse.existsSync(folder)) fse.mkdirSync(folder);
  var form = new formidable.IncomingForm();
  form.parse(req, function (err, fields, files) {
    console.log(files);

    if (
      (files.img0.size > 0 && files.img0.type == "image/png") ||
      files.img0.type == "image/jpeg"
    )
      req.session.user.img0 =
        "/img/user/" + req.session.user.login + "/" + files.img0.name;

    profileModel
      .updateImg(user_id, user_login, folder, files)
      .then((result) => {
...
router.post("/updateTags", function (req, res) {
  ...
  tags.forEach(function (element) {
    ...
    if (validTag == true) {
      tagModel.tagExists(element).then((result) => {
        if (result) {
          ...
          tagModel
            .userTagExists(user_id, result[0].id)
            .then((respuesta) => {
              if (!respuesta) tagModel.addUserTag(user_id, result[0].id);
            })
          ...
          tagModel
            ...
            .then((result) => {
              ...
              Promise.all(result).then((valores) => {
                if (valores[1]) tagModel.addUserTag(user_id, valores[1].id);
              });
              //res.redirect('/user/profile');
            })
            .catch(function (error) {
    ...
    } else req.session.user.error = "Le format des tags n'est pas valide";
  });
  res.redirect("/profile");
});
...
router.get("/user/:login", function (req, res) {
  ...
  userModel
    ...
    .then((result) => {
      ...
      } else res.redirect("/user/profile"); //hacer una mejor redirección
    })
    .catch(function (err) {
      console.log(err);
      res.redirect("/");
  ...
  userModel
    ...
    .then((localizacion) => {
      if (localizacion.summary.numResults == 1) {
        ...
        userModel
          .setLatLon(coordenadas.lat, coordenadas.lon, direccion, user_id)
          .then((latLonOk) => {
            if (latLonOk) {
              res.send({
                address: direccion,
              ...
              res.send({
                address: "L'address n'est pas valide",
                lat: "undefined",
                lon: "undefined",
              });
          })
...
router.get("/like/:login", function (req, res) {
  ...
  userModel.getIdUser(username).then((result) => {
    if (
      ...
      user_id != result.id &&
      result.img0 != "" &&
      result.img0 != "/img/no-img.png"
    ) {
      likesModel
        .likeExists(user_id, result.id)
        .then((resultado) => {
          if (resultado == 0) {
            likesModel
              .addLike(user_id, result.id)
              .then((results) => {
                userModel.setPopularite(5, result.id).then((popularite) => {
                  if (results && popularite)
                    res.send({ action: "added", value: 5 });
                  else res.send({ action: "removed" });
                });
              })
              .catch(function (err) {
                console.log(err);
            ...
            likesModel
              .suprimeLike(user_id, result.id)
              .then((results) => {
                userModel.setPopularite(-5, result.id).then((popularite) => {
                  if (results && popularite)
                    res.send({ action: "removed", value: -5 });
                  else res.send({ action: "added" });
                });
              })
              .catch(function (err) {
                console.log(err);
  ...
  userModel.getIdUser(login).then((loginOk) => {
    if (loginOk) {
      if (req.body.report == "faux") {
        profileModel
          .reportFauxExists(req.session.user.id, loginOk.id)
          .then((fauxExists) => {
            if (fauxExists) {
              res.send(false);
            } else {
              profileModel
                .addReportFaux(req.session.user.id, loginOk.id)
                .then((fauxadded) => {
                  if (fauxadded) res.send(true);
                  else res.send(false);
                });
            }
          })
        ...
        profileModel
          .reportBlockExists(req.session.user.id, loginOk.id)
          .then((blockExists) => {
            if (blockExists) {
              res.send(false);
            } else {
              profileModel
                .addReportBlock(req.session.user.id, loginOk.id)
                .then((blockadded) => {
                  likesModel
                    .suprimeLike(req.session.user.id, loginOk.id)
                    .then((removeOk) => {
                      if (blockadded) res.send(true);
                      else res.send(false);
                    })
                    .catch(function (err) {
                      console.log(err);
  ...
  userModel.getUserById(req.session.user.id).then((user) => {
    if (user) {
      if (user.lat && user.lon) {
        res.send({
          key: "pk.917b163c0e500ff60c7679805ad6b270",
          lat: user.lat,
          lon: user.lon,
        });
      } else res.send(false);
  ...
  try {
    ...
    userModel
      ...
      .then((latLon) => {
        if (latLon) {
          userModel
            .setLatLon(latLon.lat, latLon.lon, latLon.city, user_id)
            .then((latLonOk) => {
              if (latLonOk) {
                userModel.getUserById(user_id).then((result) => {
                  delete result.passwd;
