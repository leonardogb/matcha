module.exports = {
  ...
  isValidNom: (nombre, apellido) => {
    ...
    if (
      ...
      if (
        !/^[a-zA-Zàáâäèéêëęîïíìôöòóûüùúñ]+([ -][a-zA-Za-zA-Zàáâäèéêëęîïíìôöòóûüùúñ]+)?$/g.test(
          nombre
        ) ||
        !/^[a-zA-Za-zA-Zàáâäèéêëęîïíìôöòóûüùúñ]+([ -][a-zA-Za-zA-Zàáâäèéêëęîïíìôöòóûüùúñ]+)?$/g.test(
          apellido
        )
      )
        error.push(
          "Votre prénom et nom peuvent avoir des lettres, un espace et un tiret"
        );
    }
    return error.length > 0 ? error : true;
  },
  ...
  isValidUpdate: (tab) => {
    ...
    if (
      ...
      if (
        tab.orientation != "Bisexuel" &&
        tab.orientation != "Homosexuel" &&
        tab.orientation != "Hétérosexuel"
      )
        error.push(
          "L'orientation sexuelle peut être Bisexuel, Homosexuel ou Hétérosexuel"
        );
  ...
  isValidImg: (path) => {
    ...
    if (
      !/^\/img\/user\/[a-zA-Z0-9\-\_]+\/[a-zA-Z0-9\-\_]+\..{3}$|\/img\/no-img.png/g.test(
        path
      )
    )
      return false;
