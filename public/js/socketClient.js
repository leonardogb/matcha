...
$(".blockFixe").on("keydown", "textarea", function (e) {
  if (e.which == 13) {
    ...
    if (text !== "") {
      ...
      if (text && user) insertChat(text, user);
      $(this).val("");
    }
  }
});
...
socket.on("newMessage", (data) => {
  if (data.msg != "") {
      ...
      var elemento =
        ...
        `</abbr>
                    ...
                    <div class="round hollow text-center botonChat">
                        <a class="addClass"><span class="fa fa-comment"></span>` +
        data.user +
        `</a>
                    </div>
                </div>`;
      ...
      `
            <div>
                <div class="direct-chat-info clearfix">
                    <span class="direct-chat-name-right pull-right">` +
      data.user +
      `</span>
                </div>
                <!-- /.direct-chat-info -->
                <img alt="message user image" src="` +
      data.userImg +
    ...
    $(
      "#" + data.user + " .chat-popup .popup-messages .direct-chat-messages"
    ).append(elem);
    var parteChat = $("#" + data.user + " .chat-popup .popup-messages");
    parteChat.animate({ scrollTop: parteChat.prop("scrollHeight") }, "slow");
...
function insertChat(text, user) {
  ...
  var userImg = $(
    "#" + user + " .popup-box .popup-head .popup-head-left .userImg"
  ).attr("src");
  var destinoImg = $(
    "#" + user + " .popup-box .popup-head .popup-head-left .dstImg"
  ).attr("src");
  ...
  if (message != "") {
    ...
    $(
      "#" + destinatario + " .chat-popup .popup-messages .direct-chat-messages"
    ).append(elemento);
