...
var userM = {
  ...
  emailExists: function (email) {
    return new Promise(function (resolve, reject) {
      database.query(
        ...
        function (err, results) {
          if (err) reject(err);
          if (results) resolve(results.length);
          else resolve(false);
        }
      );
    });
  ...
  addUser: function (tab) {
    if (
      ...
      return new Promise(function (resolve, reject) {
        database.query(
          "INSERT INTO `users` (`login`, `mail`, `prenom`, `nom`, `passwd`, `cle`) VALUES (?)",
          [tab2],
          function (err, results) {
            if (err) reject(err);
          ...
          var mailOptions = {
            ...
            html:
              ...
              '. </br>\
                            ...
                            <a href="http://localhost:8080/user/activation/' +
              encodeURI(tab.login) +
              "/" +
              encodeURI(cle) +
              '">Verifier compte</a>\
  ...
  userActivation: function (login, cle) {
    ...
    return new Promise(function (resolve, reject) {
      database.query(
        "SELECT cle, active FROM `users` WHERE login=?",
        username,
        function (err, results) {
          if (err) reject(err);
          if (results.length == 1) {
          ...
          } else reject("Le lien d'activation n'est pas valide.");
        }
      );
    }).then(function (result) {
      if (result == true) {
        database.query(
          "UPDATE `users` SET cle = null, active = 1 WHERE login=?",
          username,
          function (err, results) {
            if (err) reject(err);
          }
  ...
  reinitMDP: function (mail) {
    return new Promise(function (resolve, reject) {
      ...
      database.query(
        "UPDATE `users` SET passwd = ? WHERE mail = ?",
        [mdpHash, mail],
        function (err, MdpOk) {
          if (err) reject(err);
          if (MdpOk) resolve(mdp);
          else resolve(false);
        }
      );
    });
        ...
        html:
          `<html>
                ...
                <body
                    ...
                    <p>Mot de passe temporaire: <strong>` +
          mdp +
          `</strong></p>
  ...
  auth: function (username, passwd) {
    ...
    return new Promise(function (resolve, reject) {
      database.query(
        "SELECT id, login, prenom, nom, passwd, mail, active, img0 FROM users WHERE login=?",
        username,
        function (err, results) {
          if (err) reject(err);
          ...
          if (results[0]) {
            if (results[0].active == 1) {
              bcrypt.compare(passwd, results[0].passwd, function (err, res) {
                if (res == true) resolve(results[0]);
                else reject("Le login ou le mot de passe n'est pas valide.");
              });
            } else reject("Vous devez activer votre compte.");
          } else reject("Êtes-vous inscrit ?");
        }
      );
    });
  },
  verifMDP: function (username, mdp) {
    return new Promise(function (resolve, reject) {
      database.query(
        "SELECT passwd, active FROM `users` WHERE login=?",
        username,
        function (err, results) {
          if (err) reject(err);
          if (results[0]) {
  ...
  getUserByUsername: function (username) {
    return new Promise(function (resolve, reject) {
      database.query(
        ...
        function (err, results) {
          if (err) reject(err);
          if (results && results.length == 1) resolve(results[0]);
          else resolve(false);
        }
      );
    });
    ...
    return new Promise(function (resolve, reject) {
      database.query(
        ...
        function (err, results) {
          ...
          else reject("Verifica la base de datos.");
        }
      );
    });
  },
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (sex == "Autre") {
        if (user1_sex == "Masculin") {
          sql =
            "SELECT * FROM users WHERE complet = 1 AND id != ? AND ((genre = 'Masculin' AND orientation != 'Hétérosexuel') OR (genre = 'Féminin' AND orientation != 'Homosexuel'))";
          params = [id_user];
        } else if (user1_sex == "Féminin") {
          sql =
            "SELECT * FROM users WHERE complet = 1 AND id != ? AND ((genre = 'Masculin' AND orientation != 'Homosexuel') OR (genre = 'Féminin' AND orientation != 'Hétérosexuel'))";
          params = [id_user];
        }
      } else {
        sql =
          "SELECT * FROM users WHERE complet = 1 AND genre=? AND id != ? AND (orientation = ? OR orientation = 'Bisexuel')";
  ...
  getUsersLimits: function (
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (sex == "Autre") {
        if (user1_sex == "Masculin") {
          sql =
            "SELECT * FROM users WHERE complet = 1 AND id != ? AND age >= ? AND age <= ? AND popularite >= ? AND popularite <= ? AND ((genre = 'Masculin' AND orientation != 'Hétérosexuel') OR (genre = 'Féminin' AND orientation != 'Homosexuel'))";
          params = [id_user];
        } else if (user1_sex == "Féminin") {
          sql =
            "SELECT * FROM users WHERE complet = 1 AND id != ? AND age >= ? AND age <= ? AND popularite >= ? AND popularite <= ? AND ((genre = 'Masculin' AND orientation != 'Homosexuel') OR (genre = 'Féminin' AND orientation != 'Hétérosexuel'))";
          params = [id_user];
        }
        ...
        sql =
          "SELECT * FROM users WHERE complet = 1 AND (orientation = ? OR orientation = 'Bisexuel') AND age >= ? AND age <= ? AND popularite >= ? AND popularite <= ? AND genre = ? AND id != ?";
      ...
      if (tri == "age") sql += " ORDER BY age";
      else if (tri == "location") sql += " ORDER BY location";
      else if (tri == "popularite") sql += " ORDER BY popularite";
      database.query(sql, params, function (err, users) {
        if (err) reject(err);
        if (users) resolve(users);
        else resolve(false);
  ...
  getUsersLimits2: function (id_user, age, popul, tri) {
    return new Promise(function (resolve, reject) {
      ...
      sql =
        "SELECT * FROM users WHERE complet = 1 AND age >= ? AND age <= ? AND popularite >= ? AND popularite <= ? AND id != ?";
  ...
  getIdUser: function (username) {
    return new Promise(function (resolve, reject) {
      if (username) {
        database.query(
          "SELECT id, img0 FROM users WHERE login = ?",
          username,
          function (err, resultado) {
            if (err) reject(err);
            if (resultado && resultado.length == 1) resolve(resultado[0]);
            else resolve(false);
          }
        );
      } else resolve(false);
  ...
  setPopularite: function (valeur, id_user) {
    return new Promise(function (resolve, reject) {
      database.query(
        "UPDATE users SET popularite = popularite + ? WHERE id = ?",
        [valeur, id_user],
        function (err, result) {
          if (err) reject(err);
  ...
  setVisite: function (userId, estado) {
    return new Promise(function (resolve, reject) {
      if (estado == "on") {
        database.query(
          "UPDATE users SET visite = 'online' WHERE id = ?",
          userId,
          function (err, result) {
            if (err) reject(err);
            if (result && result.affectedRows == 1) resolve(true);
        ...
        database.query(
          "UPDATE users SET visite = ? WHERE id = ?",
          [fecha, userId],
          function (err, result) {
            if (err) reject(err);
    ...
    return new Promise(function (resolve, reject) {
      ...
      axios(
        "https://api.tomtom.com/search/2/geocode/" +
          address +
          ".JSON?key=14hKz7D9ojvNcaHNaBAqvePAM4QTu6ki&limit=1"
      )
        .then((response) => {
  ...
  insertNewUser: function (user) {
    return new Promise(function (resolve, reject) {
      database.query(
        "INSERT INTO users (login, prenom, nom, passwd, mail, active, genre, age, orientation, bio, img0, ville, lat, lon, location, popularite, complet, visite) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          user[0],
          user[1],
          user[2],
        ...
        function (err, resp) {
          if (err) reject(err);
          if (resp && resp.affectedRows) resolve(true);
          else resolve(false);
        }
      );
    });
