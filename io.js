          ...
          if (userId) {
            ...
            Promise.all(usuarios).then((resultado) => {
              if (resultado[0] && resultado[1]) {
                var likes = [];
                likes.push(
                  likesModel.likeExists(resultado[0].id, resultado[1].id)
                );
                likes.push(
                  likesModel.likeExists(resultado[1].id, resultado[0].id)
                );
                Promise.all(likes).then((likes) => {
                  console.log(likes);
                  if (likes[0] && likes[1] && likes[0] == 1 && likes[1] == 1) {
                    profileModel
                      .reportBlockExists(resultado[1].id, resultado[0].id)
                      .then((bloqueado) => {
                        if (!bloqueado) {
                          if (destId) {
                            data["date"] = date;
            ...
            if (userid1.id) {
              ...
              Promise.all(likes).then((lik) => {
                if (notif.notif == "addLike") {
                  if (lik[0] == 1 && lik[1] == 1) {
                    profileModel
                      .reportBlockExists(userid.id, userid1.id)
                      .then((bloqueado) => {
                        if (!bloqueado) {
                          notifModel
                            .addNotif(
                              userid.id,
                              "Bravo, vous matchez avec " + socket.user
                            )
                            .then((resp) => {
                              //emit😊
                              if (resp)
                                socket
                                  .to(users[notif.userDst])
                                  .emit(
                                    "newNot",
                                    "Bravo, vous matchez avec " + socket.user
                                  );
                      ...
                      .then((bloqueado) => {
                        if (!bloqueado) {
                          notifModel
                            .addNotif(
                              userid.id,
                              "Vous avez un nouveau like de " + socket.user
                            )
                            .then((resp) => {
                              //emit
                              if (resp)
                      ...
                      .then((bloqueado) => {
                        if (!bloqueado) {
                          notifModel
                            .addNotif(
                              userid.id,
                              "Vous ne matchez plus avec " + socket.user
                            )
                            .then((resp) => {
                              //emit 😢
                              if (resp)
                    ...
                    .then((bloqueado) => {
                      if (!bloqueado) {
                        notifModel
                          .addNotif(
                            userid.id,
                            socket.user + " a visité ton profil"
                          )
                          .then((resp) => {
                            if (resp)
                              socket
